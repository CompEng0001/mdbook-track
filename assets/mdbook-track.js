(() => {
  ...
  function pageState(state, pageId) {
    ...
    if (
      !state.pages[pageId].items ||
      typeof state.pages[pageId].items !== "object"
    ) {
      state.pages[pageId].items = {};
    }
  ...
  function renderOverview(overview, manifest) {
    ...
    for (const [section, entries] of groups) {
      ...
      sectionCount.textContent = `${sectionCompleted} / ${sectionTotal} items`;
      ...
      for (const { page, stats } of entries) {
        ...
        row.setAttribute(
          "aria-label",
          `${page.title}: ${stats.completed} of ${stats.total} items completed`,
        );
  ...
  function refreshOverviews(bookId = null) {
    ...
    for (const overview of document.querySelectorAll(
      ".mdbook-track-overview",
    )) {
